    ...
    <div class="space-y-6">
        ...
        <flux:card>
            ...
            <form wire:submit="updateStatus" class="space-y-4">
                <flux:select wire:model="status" label="Conversation Status">
                    @foreach($statuses as $statusOption)
                        <option value="{{ $statusOption->value }}">{{ ucwords(str_replace('_', ' ', $statusOption->value)) }}</option>
                    @endforeach
                </flux:select>
                ...
                <div class="space-y-4">
                    @foreach($conversation->documents as $document)
                        <div wire:key="document-{{ $document->id }}" class="border border-zinc-200 dark:border-zinc-700 rounded-lg p-4">
                            ...
                            <div class="mt-4 bg-zinc-50 dark:bg-zinc-900 rounded p-4 overflow-x-auto">
